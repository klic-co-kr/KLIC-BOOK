# 인포그래픽 저작 가이드 (Phase 2 — flow·cards·matrix)
...
| 필드 | 필수 | 규약 |
|---|---|---|
| `layout` | O | `flow`·`cards`·`matrix` (구별칭 `process`→flow, `principles`·`dashboard`→cards, `quadrant`→matrix 정성 — 자동 변환) |
...
- **숫자는 evidence가 필요하다.** title·kicker·steps·cards·matrix 셀 문구에
  아라비아숫자 토큰(`3`, `50%`, `1.5`)이 있으면 evidence 필수이고, I1이 각
  숫자가 §N 원문에 부분 문자열로 존재하는지 **자동 교차검증**한다. 면제:
  `제N장/제N절` 서수, 원형 숫자 ①②③, 한글 수사. `thesis`·`note`는 검사
  제외(편집자 문구). 정성 매트릭스 축 라벨(`x_axis`/`y_axis`)도 검사 대상이다.
...
## 예산 치트시트 (practical 기준 — 골든 교정 1주기 실측 반영)
...
(4열) · 정성 셀 119.2pt · 레인 셀 66.2pt(3열) · actor 열 60pt.
...
## 골든 교정 절차 (스펙 §7 — 1주기 완료 2026-08-22, archetype 추가·계수 드리프트 의심 시 반복)

1. `tests/fixtures/infographic/calib-cards.md`(경계 문구 고정 세트)를
