...
#import "openers.typ": opener-enabled, opener-paper, opener-brand, openers
...
// 오프너 변조용 H1 발생 카운터 — 무번호 장(들어가며·맺음말)도 센다.
#let _occ = counter("klic-h1-occ")
...
#let base(body) = {
  ...
  show heading.where(level: 1): it => {
    ...
    if opener-enabled [
      #page(margin: 0pt, fill: opener-paper, header: none, footer: none)[
        ...
        #pad(top: mm(tokens.margin.top_mm + 8), bottom: mm(24),
          ...
          #text(size: pt(tokens.fonts.label.size_pt), tracking: 2pt,
            fill: opener-brand)[#upper(eyebrow)]
          #v(9mm)
          // 고스트 장번호 — 잉크 농도 낮게(밝은 지면용)
          #if not is-part and not is-nonum [
            #text(size: pt(tokens.fonts.heading1.size_pt * 2.1), weight: "bold",
              fill: rgb(tokens.colors.ink).transparentize(86%))[#_rank1(it)]
            #v(7mm)
          ]
          #text(size: pt(tokens.fonts.heading1.size_pt), weight: "bold",
            fill: rgb(tokens.colors.ink))[#it.body]
        ]
        // 우하단 마감 — 액센트 스퀘어 + 가는 룰(표지 임프린트와 대응)
        #place(bottom + right, dx: -mm(tokens.margin.outer_mm), dy: -mm(20))[
          #align(right, stack(dir: ttb, spacing: 2.5mm,
            rect(width: mm(4), height: mm(4), fill: opener-brand),
            rect(width: mm(30), height: mm(0.7), fill: opener-brand)))
        ]
      ]
    ] else [
