...
**`cover: auto`(또는 생략)** — 5변형 벡터 표지 문법(골격)에서 자동 선택한다.
변형은 제목 해시로 결정적 분배(같은 책 = 같은 표지, 책마다 상이)하며
`cover_variant: 1|2|3|4|5`로 직접 지정할 수 있다.
V1 비대칭 좌측(엣지 바·두 원·모티프 스왑) · V2 이중 프레임 문고형(괘선·중앙
타이포·코너 틱) · V3 수평 밴드형(브랜드 밴드 타이틀·원 클러스터) ·
V4 엣지 포인트(우상단 하프톤 도트 필드 감쇠 + 도트 언더라인) ·
V5 위계 격자형(외곽 프레임·좌측 모듈 축·대제목 하단 앵커).
해시 분배는 V1~V4만 순환한다(`% 4 + 1`) — V5는 명시 지정 전용.
모든 변형이 마지막 단어 강조 타이포·'지음'·발행 락업을 공유하고 판형 크기에 맞춘다.

**`cover_composition: true`** — 표지 변조 프로파일(opt-in). 50종 구성
프로파일(구도 12·시각 원칙 8·출판 판면 10·격자 10·전통 구도 10) 중
하나를 제목 해시로 골라 골격에 주입한다 — 변조축: 타이틀 앵커(top/mid/
bottom)·모티프(dots/circles/lines/squares/none)·밀도·액센트·피치.
V5 전축, V1 모티프 교체, V4 밀도/피치 연동. V2/V3는 미반영(경고 로그).
꺼 두면 기존 변형 문법 그대로(기존책 표지 불변).

**`opener_composition: true`** — 장 오프너 변조(opt-in, 미지정 시
`cover_composition`을 따름). 켜면 네이비 전면 오프너 대신 밝은 오프너
(표지색 지면·우측 세로 진열 스트립·장마다 배치 변화·잉크 제목)가 된다.
표지와 같은 프로파일 계열이되 형태는 다르다.
...
md2typst 매핑: `##`→H1(`=`), `###`→H2(`==`), `####`→H3(`===`), `#####`→H4(`====`). **`#` 단독도 H1(`=`)로 변환된다.**
...
논문 기반 책의 출처 정보는 **인쇄물 안에** 실린다. `ATTRIBUTION.md`는 저장소 기록용(그림
추출 내역 등)이며 `typst-build.yaml`의 `chapters:` 밖이라 독자에게 전달되지 않는다 —
출처 표기는 원고 본문에 내장한다. 실물 원문에서 확인한 정보(저자·소속·식별자·URL)만 적는다.

- **단일 논문 정독서** — `00-들어가며.md` 첫 머리, 장 제목 바로 아래에 인용 블록:

  ```markdown
  > 원문: {논문 제목} — {arXiv ID 또는 출판 형태·연도} · {저자} ({소속})
  > 링크: {원문 URL}
  > 코드: {코드 저장소 URL}    <!-- 있을 때만 -->
  ```
...
- 실례: `books/skill-state-ko/manuscript/00-들어가며.md:3`(단일 논문),
  `books/agent-papers-2026-ko/manuscript/01-*.md`(논문집 장머리).
...
cover_variant: 5        # 선택 — 골격 지정(1~5). 미지정 시 해시 분배(V1~V4)
cover_composition: true # 선택 — 표지 변조 프로파일 50종 opt-in
opener_composition: true  # 선택 — 장 오프너 변조(미지정 시 표지 키 따름)
...
| G4 | 한글 문체 — 기계 한국어·번역투 패턴(명사형 종결·조각문·되어지·상투구·'의' 연쇄·엠대시 밀도·문두 스캐폴딩·헤지·번역투 조사 밀도·'~ 아니라' 대조 빈도). 원고 md에서 검사, [fluent-korean](https://github.com/snflkd/fluent-korean)·[polish-doc](https://github.com/albertrim/polish-doc) 규칙 기계화 | WARN |
| G5 | 콘텐츠 정합성 — 미해결 라틴 시퀀스(allowlist 외 ALL-CAPS)·판독불능 표기(○○ 등)·"N장" 교차참조·HTML 주석 잔여. 원고 md 검사. `g5_rescan_inventory`(⚠️ 미스캔 면 목록)는 경고가 아닌 보고서 자산 | WARN |
