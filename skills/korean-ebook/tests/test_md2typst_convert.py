...
def test_pipe_table_columns_numeric_narrow():
    # 본문 전체가 숫자인 열은 좁은 fr — 균등 1fr이면 텍스트 열까지
...
def test_pipe_table_text_columns_wider_than_numeric():
    ...
    # 토큰(Qwen3-8B 등)이 셀 밖으로 넘쳐 다음 열 위에 인쇄된다.
    md = ("| 방법 | 백본 | Pick | Avg |\n|---|---|---|---|\n"
...
def test_pipe_table_korean_cell_weighted_wider():
    ...
    # 토큰 구조를 같게 두면(2음절×4 vs 2자×4) 한글 셀 열이 더 넓어야 한다.
    def first_fr(md):
        import re as _re
        return float(_re.search(r"columns: \(([\d.]+)fr", convert(md)).group(1))
    ko = first_fr("| 항목 | 값 |\n|---|---|\n| 가나 다라 마바 사아 | 47.9 |\n")
    en = first_fr("| 항목 | 값 |\n|---|---|\n| ab cd ef gh | 47.9 |\n")
    assert ko > en
