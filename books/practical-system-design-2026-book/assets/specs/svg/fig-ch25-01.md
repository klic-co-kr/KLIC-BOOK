...
# fig-ch25-01 — SVG 제작 명세

## 목적

클라이언트 전체 deadline이 gateway·service·DB 호출의 남은 예산으로 줄어드는 흐름을 보여준다.
...
- `aws-timeouts-retries` — Timeouts, retries, and backoff with jitter
- `google-sre-overload` — Site Reliability Engineering — Handling Overload
- `stripe-idempotency` — Stripe API — Idempotent requests
