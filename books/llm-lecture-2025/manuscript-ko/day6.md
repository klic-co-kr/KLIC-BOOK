...
$$GPU 메모리 사용량의 구체적 비교를 위해 7B 모델의 16-bit fine-tuning을 상정해 봅니다. 전체 파라미터 수 N_all = 7B, 부동소수점 크기 size(float) = 2byte 상황에서, Full-FT가 갱신하는 파라미터 수 N_train = 7B, PEFT가 갱신하는 파라미터 수 N_train \approx 1M로 잡으면 각 항목은 다음과 같이 추정됩니다. Model Loading(size(float) × N_all)은 양쪽 모두 약 13GB, Backward/Gradients(2 × size(float) × N_train)은 Full-FT 약 26GB·PEFT 약 2MB, Optimizer/Adam(size(float) × N_train)은 Full-FT 약 13GB·PEFT 약 4MB입니다. Forward 분이나 라이브러리 확보 영역 등 batch size에 비례하여 증가하는 부분(α)을 더하면, 총 GPU 메모리 사용량은 Full-FT 약 52GB+α, PEFT 약 13GB+α로 큰 차이가 납니다$$
...
$$**Adapter(2019)**[16]는 Transformer 내부에 학습 가능한 Adapter 모듈을 추가하여 학습합니다. Adapter는 단순한 MLP 구조를 가지며, 추가 위치가 다른 아종도 존재합니다(예: Parallel Adapter[17]는 병렬적으로 Adapter를 추가). 장점은 Full-FT 대비 10-1에서 10-2 정도로 작은 학습 파라미터 수로 Full-FT와 동등한 정확도를 달성한다는 점, 그리고 Adapter만 보존하면 되어 유연하게 교체 대응이 가능하다는 점입니다. 단점은 Adapter가 추가됨으로써 추론에 오버헤드가 발생한다는 것입니다$$
...
$$**LoRA(2021)**[14]는 Reparametrization형의 대표 기법입니다. fine-tuning으로 갱신된 가중치 W는 일반적으로 원래 가중치 W0와 증분 가중치 ΔW의 합으로 표현 가능합니다. LoRA에서는 이 증분 가중치 ΔW를 두 저랭크 행렬 A(d × r)와 B(r × d)의 곱으로 삼고, 그것들에 대해 학습을 실시합니다. 사전학습 모델 선형 층의 가중치는 고정해 두고, A는 정규 난수로, B는 영행렬로 초기화하여 두 경로의 결과를 합산해 다음 층으로 전달합니다. 장점은 Full-FT 대비 10-2에서 10-4 정도로 작은 학습 파라미터 수로 Full-FT와 동등한 정확도를 달성한다는 점, 그리고 추론 시에는 얻어진 가중치를 원래 가중치에 미리 더해두면 오버헤드가 발생하지 않는다는 점입니다. 단점은 특히 난이도가 높은 태스크(예: GSM8k, 수학적 추론)에서 Full-FT 대비 현저한 성능 열위가 발생할 수 있다는 점입니다[20]$$
