...
$$이런 일은 어떻게 실현되는 걸까요? 단어의 열(문장)을 x1, x2, ⋯, x_𝐿이라 할 때, 언어 모델은 그 생성 확률 p(x1, x2, ⋯, x_𝐿)을 할당하는 확률 모델 p로, 자연스러운 문장일수록 높은 확률을 줍니다$$
...
```
$$p(x1, x2, ⋯, x_𝐿) = p(x1) · p(x2|x1) · ⋯ · p(x_𝐿|x1, x2, ⋯, x_{𝐿−1})$$
```
