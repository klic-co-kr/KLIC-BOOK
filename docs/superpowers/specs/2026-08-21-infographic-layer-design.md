# korean-ebook-typst 인포그래픽 레이어 설계 (개정 2판)

- 날짜: 2026-08-21 (초판) / 2026-08-21 (개정 2판 — 적대 검토 3관점 24건 반영)
- 상태: 승인 대기
...
[1] md2typst.py (기존 CLI에 --fences-out <path> 인자 추가)
      변환 스텝 0.5(코드펜스 stash) 이전에 ```infographic 펜스를 먼저 추출:
        ...
        - 펜스 YAML 원문 + 순번을 --fences-out 사이드 파일에 기록
[2] build.py
...
```yaml
layout: process        # 필수. 9종 키워드 + composite (§3.5 별칭 참조)
...
YAML 파싱 실패: 챕터·펜스 순번·라인·키 경로 명시.
...
- 공간 부족 시: 보조 모듈 축소 → 여전히 초과면 **모듈 단위 분할 에러**.
  메시지 스키마: `{모듈 슬롯} {layout} 측정높이 {h}pt > 배분 {H}pt —
  보조 모듈 n을 별도 펜스로 분할 권장`. 글자·화살표 축소 금지.
...
- 도식 제목 = 본문 H2급 · 카드 제목 = 본문+1pt · 카드 본문 = 본문−1pt ·
  캡션 = 기존 `label` 크기. **도식 텍스트 크기는 본문 크기와 ±0.3pt
  차이가 보장되도록 설계**한다(G3 글자수 밴드 필터가 도식 텍스트를
  우연이 아닌 설계로 제외시키는 불변식 — I1이 emit 코드에서 검증).
...
- 큰 도식이 다음 페이지로 밀릴 때 앞면 공백은 허용된 비용이다(최대
  프레임 85% 상한이 공백 상한을 간접 제한). `figure(placement: none)`
  기본 — 본문 흐름 유지. 배치 자동 최적화는 하지 않는다(결정론).
...
8. **펜스 위장 감지**: 미등록 펜스 언어라도 내용 YAML에 `layout:` 키가
...
- **검수 렌더 절차(구 계약 회수)**: 도식 페이지를 160~180 DPI로 별도
  렌더링. build_report.json에 `infographic_pages`(도식 수·페이지
...
- flow 초과 시 **결정론 규칙**: 2행 랩 시 전 카드가 예산 안에 들면
  랩, 아니면 세로 변형. (선택 주체 없음 — 초판 "랩 또는 세로" 모순 수정)
...
| layout | 배치 산술 |
|---|---|
| `flow` | 가로 n카드, 간격 24~32pt. 폭 부족 시 §6.2 결정론 규칙 |
...
5. **composite + 가이드 완성** — 복합 씬, authoring.md 전체
   (라우팅 표 9종·검수 절차·치트시트 전판형), SKILL.md 요약 갱신,
   통합 테스트 마무리.
